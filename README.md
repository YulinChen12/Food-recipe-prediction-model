...
Since p_value ≥ 0.05, we **fail to reject** the null. we could conclude that there is not enough statistical evidence to reject the null hypothesis, implying that any observed differences in RMSE between the two groups might be due to chance. This outcome does not align with our first assumption that calories might be correlated to the “Cheese” feature. That is why we then created a new column named high_caloried_count to better predict calories. **However, it does not imply absolute certainty.** Since the statistical test result can be influenced by various factors. 
